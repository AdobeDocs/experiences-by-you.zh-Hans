...
1. **回访标准：** 访问
1. **粒度：** 月
1. **设置：** 滚动计算\*\*允许您根据前一列而不是所包含的列计算维系率。 因此，这意味着每个月都会包含用户\*\*
...
注意：同类群组分析不能用于计算量度（例如转化率）或非整数量度（例如收入）。 只有可以在区段中使用的量度才能在同类群组分析中使用，并且它们一次只能递增 1。
...
自定义维度同类群组：创建基于所选维度的同类群组，而不是基于时间的同类群组（默认）。许多客户想要按时间以外的其他方式分析他们的同类群组，现在，通过新的自定义维度同类群组功能，可以灵活地根据他们所选的维度构建同类群组。在中使用营销渠道、营销活动、产品、页面、区域或任何其他维度 [!DNL Adobe Analytics] 以显示维系率根据这些维度的不同值有何变化。  

自定义Dimension同类群组区段定义仅将维度项用作包含时段的一部分，而不是回访定义的一部分。

选择自定义维度同类群组选项后，您可以将所需的任何维度拖放到拖放区域中。这允许您比较同一时间段内的类似维度项目。 例如，您可以并排比较各个城市的绩效

侧、产品、营销活动等。 它会返回您的前14维项目。 但是，您可以使用过滤器（通过将鼠标悬停在拖动的维度的右侧来访问过滤器）来仅显示所需的维度项目。 自定义维度同类群组不能与延时表功能一起使用。
...
![詹妮弗·亚森达](assets/jennifer-yacenda.png)

**詹妮弗·亚森达**、万豪的高级Director
